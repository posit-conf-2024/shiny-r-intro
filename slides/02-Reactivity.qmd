...
## Reactive expression to reactive graph
...
#| eval: false
#| file: "../demos/demo02.R"
#| code-line-numbers: "|6-13|30-33|32|42-46|45"
```
...
One aspect of the improved code that you might be unfamiliar with is the use of `.data[[input$var]]` within the `renderPlot()` reactive expression. This is a useful feature from `rlang` that can be used across much of the tidyverse to reference columns in a data frame using a variable. 

Specifically it is an alternative that is useful for avoiding some of the complexity around "non-standard evaluation" with `rlang` (e.g. `{{`, `!!`, `enquo()`, etc.) when working with functions built with the tidy eval framework (e.g. `dplyr` and `ggplot2`).
...
Starting with the code  in `exercises/ex03.R` (based on `demo02.R`'s code) add a `tableOutput()` labeled `minmax` to the app's `mainPanel()`.
...
- `lubridate::year()` will be useful along with `dplyr`s `group_by()` and `summarize()`.
...
```{r}
#| eval: false
#| code-line-numbers: "|45-48|51,58"
#| file: ../demos/demo04.R
```

## Reactive expressions

This is an example of a "reactive conductor" as it exists in between sources (e.g. an input) and endpoints (e.g. an output).

As such, a `reactive()` depends on various upstream inputs and then has 1 or more downstream outputs.
...
- Expressions written in the same way as `render*()` functions
...
```{r}
#| eval: false
#| code-line-numbers: "|19-22|23-25|40-48"
#| file: ../demos/demo05.R
```
...
Reactive graphs are meant to be acyclic, that is they should not have circular dependencies. The use of observers can introduce cycles which can then lead to infinite loops, see the following example:
